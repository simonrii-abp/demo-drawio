...
Open [`index.html`](index.html) directly in a browser. Internet access is required because the editor iframe is loaded from `https://embed.diagrams.net`.

1. Select **Edit diagram**.
2. Change the diagram in the embedded editor.
3. Select **Save** or **Save & Exit** in diagrams.net.
4. Download the updated XML or editable SVG from the host application.

The browser demo saves XML to `localStorage`; it does not write back to this repository. Replace files in `diagrams/` with downloaded versions when you want to commit changes.
